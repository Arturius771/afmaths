...
from operation import CUBE, SQUARE, add, divide, dot_product_3d, exponentiate, multiply, square_root, subtract, vector_cross_multiplication_3d, vector_magnitude_3d, vector_multiplication_3d
...
def velocity_for_altitude(orbit_radius: float, gravitational_parameter: float = 3.986005e+14) -> float:
...
def eccentricity_from_angular_momentum_vector(angular_momentum_vector: list[float], semi_major_axis: float, gravitational_parameter: float = 398600) -> float:
...
def mean_anomaly_at_time_offset(mean_anomaly: float, time_offset_s: float, mean_motion: float) -> float:
...
def eccentric_anomaly_from_true_anomaly(true_anomaly: float, eccentricity: float) -> float:
    return 2 * math.atan2(
    math.sqrt(1 - eccentricity) * math.sin(true_anomaly / 2),
    math.sqrt(1 + eccentricity) * math.cos(true_anomaly / 2))

def eccentric_anomaly_solved(iteration_func, eccentricity, mean_anomaly, tol=1e-6, max_iter=100):
    history = []
...
def orbital_elements_from_state_vectors(position_vector: list[float], velocity_vector: list[float], gravitational_parameter: float = 398600) -> dict:
    ...
    eccentricity = eccentricity_from_angular_momentum_vector(angular_momentum_vector, semi_major_axis, gravitational_parameter)
...
def newton_iteration(E_i, eccentricity, mean_anomaly):
    return E_i - (E_i - eccentricity * math.sin(E_i) - mean_anomaly) / (1 - eccentricity * math.cos(E_i))
...
def orbit_state_vector_prediction_from_orbital_elements(
    ...
    perifocal_position_vector = vector_multiplication_3d(
        [
            math.cos(true_anomaly_at_offset),
    ...
    p = [
        ...
        - math.sin(argument_of_perigee) * math.cos(inclination) * math.sin(right_ascension),

        math.cos(argument_of_perigee) * math.sin(right_ascension)
        + math.sin(argument_of_perigee) * math.cos(inclination) * math.cos(right_ascension),
    ...
    q = [
        ...
        - math.cos(argument_of_perigee) * math.cos(inclination) * math.sin(right_ascension),

        -math.sin(argument_of_perigee) * math.sin(right_ascension)
        + math.cos(argument_of_perigee) * math.cos(inclination) * math.cos(right_ascension),
...
if __name__ == '__main__':
    ...
    print(orbit_state_vector_prediction_from_orbital_elements({"inclination": math.radians(98.371), "right_ascension": math.radians(120.534), "argument_of_perigee": math.radians(10.598), "semi_major_axis": 6878.1, "eccentricity": 10e-5, "true_anomaly": 2.8022276030554347}, 1800, None))
    ...
    print(orbit_state_vector_prediction_from_orbital_elements(orbital_elements_from_state_vectors([10000,40000,-5000], [-1.5, 1, -0.1]), 1000000))